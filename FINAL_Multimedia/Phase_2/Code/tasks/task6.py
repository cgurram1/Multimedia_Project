...
def task6_execution(query_feature_model, k, dimredtech):
    if dimredtech == 1:
        data_matrix = np.loadtxt(f"{os.path.join(os.getcwd(), f'../image_image_sm_matrices/image_image_sm_{query_feature_model}.csv')}", delimiter=',')
        ...
        file_path_ls = f"{os.path.join(os.getcwd(), f'../outputs/latent_semantics/{dimredtech}_{query_feature_model}_{k}_iism.json')}"
        with open(file_path_ls, 'w') as json_file:
            json.dump(ls_dict, json_file)
        svd.calculateImageIDWeightPairs(latent_semantics, query_feature_model, k, "iism")
        ...
        with open(file_path_ls, 'w') as json_file:
            json.dump(ls_dict, json_file)
        nnmf.calculateImageIDWeightPairs(latent_semantics, query_feature_model, k, "iism")
        return latent_semantics
        ...
        with open(file_path_ls, 'w') as json_file:
            json.dump(ls_dict, json_file)
        lda.calculateImageIDWeightPairs(query_distribution, query_feature_model, k, "iism")
        return latent_semantics
        ...
        with open(file_path_ls, 'w') as json_file:
            json.dump(ls_dict, json_file)
        kmeans.calculateImageIDWeightPairs(latent_semantics, query_feature_model, k, "iism")
        return latent_semantics
...
def task6(): 
    ...
    return task6_execution(query_feature_model, k, dimredtech)
