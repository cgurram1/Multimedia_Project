...
db = cl["caltech101db"]
collection = db["phase2trainingdataset_final"]
...
def extract_RESNET_features(dataset,collection):
    ...
    for i,(tensor,label) in enumerate(dataset):
        if i%2==0:
            ...
            collection.update_one({"image_id": i },{ "$set" : { "resnet50_softmax_feature_descriptor" : feature.detach().tolist()[0]}})
...
def generate_resnet_softmax_representatives():
    for label in range(101):
        ...
        for image in label_images:
            resnet_softmax_vectors.append(np.ravel(image['resnet50_softmax_feature_descriptor']))
        ...
        rep_collection.update_one({"label": label},{"$set":{"resnet50_softmax_rep_image_id": closest_image_id_resnet,"resnet50_softmax_rep_image": closest_image_vector_resnet.tolist()}})

def task2b(query_features, k):
    feature_list = []
    feature_list=[i["resnet50_softmax_feature_descriptor"] for i in collection.find({}, {"resnet50_softmax_feature_descriptor": 1}) ]
    ...
    for rep in representatives:
        temp = 0
        temp = sqrt(js_divergence(np.array(rep["resnet50_softmax_rep_image"]),np.array(query_features)))
